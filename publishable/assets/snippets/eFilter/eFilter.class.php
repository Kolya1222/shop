...
class eFilter
{
    ...
    public $multiTV_structures = array();
    ...
    {
        $logFile = MODX_BASE_PATH . 'efilter_debug.log';
    ...
    public function renderFilterBlock($filter_cats, $filter_values_full, $filter_values, $filters, $config = '')
    {
        ...
        foreach ($filter_cats as $cat_name => $tmp) {
            ...
            foreach ($tmp as $tv_id => $tmp2) {
                $filterName = isset($filters[$tv_id]['name']) ? $filters[$tv_id]['name'] : '';
                $captionName = isset($tv_captions[$tv_id]) ? $tv_captions[$tv_id] : '';
                $filters[$tv_id]['name'] = !empty($filterName) ? $filterName : (!empty($captionName) ? $captionName : '');
                $filters[$tv_id]['name'] = $this->translate($filters[$tv_id]['name']);
                if (isset($filter_values_full[$tv_id])) {
                    // Проверяем, есть ли составные ключи (для MultiTV)
                    $hasCompositeKeys = false;
                    foreach (array_keys($filter_values_full[$tv_id]) as $key) {
                        if (strpos($key, '::') !== false) {
                            $hasCompositeKeys = true;
                            break;
                        }
                    }

                    if ($hasCompositeKeys) {
                        // Группируем значения по имени характеристики
                        $groupedValues = array();
                        foreach ($filter_values_full[$tv_id] as $key => $data) {
                            // Проверяем, что ключ содержит разделитель
                            ...
                            list($charName, $charValue) = $parts;
                            if (!isset($groupedValues[$charName])) {
                                $groupedValues[$charName] = array();
                            }
                            $groupedValues[$charName][$charValue] = $data;
                        }

                        // Выводим каждую группу как отдельный блок
                        foreach ($groupedValues as $charName => $values) {
                            $wrapper = '';
                            $active_block_class = '';
                            $i = 0;

                            // Определяем шаблоны в зависимости от типа фильтра
                            ...
                            foreach ($values as $charValue => $data) {
                                $key = $charName . '::' . $charValue;
                                ...
                                if (isset($this->fp[$tv_id])) {
                                    $flag = false;
                                    if (is_array($this->fp[$tv_id]) && in_array($key, $this->fp[$tv_id])) {
                                        $flag = true;
                                    } else {
                                        $flag = ($this->fp[$tv_id] == $key) ? true : false;
                                    }
                                    if ($flag) {
                                        if (in_array($filters[$tv_id]['type'], ['7', '8'])) {
                                ...
                                // Определяем доступен ли элемент
                                $disabled = (!empty($filter_values) && !isset($filter_values[$tv_id][$key]) ? 'disabled' : '');

                                // Получаем количество
                                if ($disabled == '') {
                                    $count = isset($filter_values[$tv_id][$key]['count']) ?
                                        $filter_values[$tv_id][$key]['count'] :
                                        $data['count'];
                                } else {
                                    $count = $this->zero;
                                }
                                ...
                                if ($this->params['removeDisabled'] == '0' || $disabled == '') {
                                    ...
                                    // Подготавливаем параметры для шаблона в зависимости от типа
                                    $tplParams = array('[+tv_id+]', '[+value+]', '[+name+]', '[+selected+]', '[+disabled+]', '[+count+]', '[+iteration+]');
                                    $tplValues = array($tv_id, $key, $charValue, $selected, $disabled, $count, $i);

                                    // Добавляем специфичные параметры для определенных типов
                                    if (in_array($filters[$tv_id]['type'], ['7', '8'])) {
                                        $tplParams = array_merge($tplParams, ['[+label_selected+]', '[+pattern_folder+]']);
                                        $pattern_folder = ($filters[$tv_id]['type'] == '8') ? $this->pattern_folder : '';
                                        $tplValues = array_merge($tplValues, [$label_selected, $pattern_folder]);
                                    }

                                    $wrapper .= $this->parseTpl($tplParams, $tplValues, $tplRow);
                                }
                            }
                            ...
                            $output .= $this->parseTpl(
                                ...
                                array($tv_id, $this->filter_tv_names[$tv_id] ?? '', $charName, $wrapper, $active_block_class),
                                $tplOuter
                            );
                            $isEmpty = false;
                        }
    ...
    public function getFilterValues($content_ids, $filter_tv_ids = '')
    {
        $filter_values = array();
        if ($content_ids != '') {
            $sql = "SELECT * FROM " . $this->modx->getFullTableName('site_tmplvar_contentvalues') . " 
                WHERE `value` IS NOT NULL AND contentid IN (" . $content_ids . ") " .
                ($filter_tv_ids != '' ? " AND tmplvarid IN (" . $filter_tv_ids . ")" : "");
            $q = $this->modx->db->query($sql);

            while ($row = $this->modx->db->getRow($q)) {
                $tv_id = $row['tmplvarid'];
                // Проверяем, является ли значение MultiTV JSON
                $isMultiTV = (strpos($row['value'], '{"fieldValue"') === 0);

                if ($isMultiTV) {
                    // Пропускаем пустые MultiTV
                    if ($row['value'] == '[]' || $row['value'] == '{"fieldValue":[]}') {
                        continue;
                    }

                    // Декодируем JSON
                    $data = json_decode($row['value'], true);
                    if (!$data || !isset($data['fieldValue']) || !is_array($data['fieldValue'])) {
                        continue;
                    }

                    // Сохраняем структуру MultiTV для этого TV
                    foreach ($data['fieldValue'] as $item) {
                        if (!is_array($item)) continue;

                        $charName = isset($item['name']) ? trim($item['name']) : '';
                        $charValue = isset($item['value']) ? trim($item['value']) : '';

                        if (empty($charName) || empty($charValue)) continue;

                        // Сохраняем в структуру
                        if (!isset($this->multiTV_structures[$tv_id][$charName])) {
                            $this->multiTV_structures[$tv_id][$charName] = array(
                                'name' => $charName,
                                'values' => array()
                            );
                        }
                        if (!in_array($charValue, $this->multiTV_structures[$tv_id][$charName]['values'])) {
                            $this->multiTV_structures[$tv_id][$charName]['values'][] = $charValue;
                        }

                        // Создаем составной ключ для фильтрации
                        $compositeKey = $charName . '::' . $charValue;

                        if (isset($filter_values[$tv_id][$compositeKey]['count'])) {
                            $filter_values[$tv_id][$compositeKey]['count']++;
                        } else {
                            $filter_values[$tv_id][$compositeKey]['count'] = 1;
                        }
                    }
                } else {
                    // Обычная обработка для не-multiTV полей
                    ...
                    if (empty($row['value']) && $row['value'] !== '0') continue;
        ...
        if (!empty($curr_filter_values)) {
            foreach ($curr_filter_values as $tv_id => $v) {
                if (isset($v['content_ids']) && $v['content_ids'] != '') {
                    ...
                    $sql = "SELECT * FROM " . $this->modx->getFullTableName('site_tmplvar_contentvalues') . " 
                        WHERE `value` IS NOT NULL AND contentid IN (" . $content_ids . ") " .
                        ($filter_tv_ids != '' ? " AND tmplvarid ={$tv_id}" : "");
                    $q = $this->modx->db->query($sql);

                    while ($row = $this->modx->db->getRow($q)) {
                        $currentTvId = $row['tmplvarid'];
                        ...
                        if ($isMultiTV) {
                            ...
                            foreach ($data['fieldValue'] as $item) {
                                ...
                                if (isset($filter_values[$currentTvId][$compositeKey]['count'])) {
                                    $filter_values[$currentTvId][$compositeKey]['count']++;
                                } else {
                                    $filter_values[$currentTvId][$compositeKey]['count'] = 1;
                                }
                            }
                        } else {
                            // Обычная обработка для не-multiTV полей
        ...
        if (!empty($this->fp)) {
            ...
            if (is_array($f)) {
                ...
                foreach ($f as $tvid => $v) {
                    ...
                    // Проверяем, является ли это поле MultiTV
                    $isMultiTVField = isset($this->multiTV_structures[$tvid]) && !empty($this->multiTV_structures[$tvid]);
                    ...
                    switch (true) {
                        ...
                        default:
                            if (is_array($v)) {
                                ...
                                foreach ($v as $k1 => $v1) {
                                    if ($v1 === '' || $v1 === '0' && !isset($this->params['allowZero'])) {
                                        unset($v[$k1]);
                                    }
                                }
                                ...
                                if ($isMultiTVField) {
                                    ...
                                    foreach ($v as $val) {
                                        ...
                                        $groupedValues[$charName][] = $charValue;
                                    }
                                    // Для каждой характеристики создаем OR условий для её значений
                                    $characteristicConditions = array();
                                    foreach ($groupedValues as $charName => $values) {
                                        $orConditions = array();

                                        foreach ($values as $charValue) {
                                            // Экранируем специальные символы для REGEXP
                                            $escapedCharName = str_replace(
                                                array('\\', '^', '$', '.', '[', ']', '|', '(', ')', '?', '*', '+', '{', '}'),
                                                array('\\\\', '\^', '\$', '\.', '\[', '\]', '\|', '\(', '\)', '\?', '\*', '\+', '\{', '\}'),
                                                $charName
                                            );
                                            $escapedCharValue = str_replace(
                                                array('\\', '^', '$', '.', '[', ']', '|', '(', ')', '?', '*', '+', '{', '}'),
                                                array('\\\\', '\^', '\$', '\.', '\[', '\]', '\|', '\(', '\)', '\?', '\*', '\+', '\{', '\}'),
                                                $charValue
                                            );

                                            $searchPattern = '"name":"' . $escapedCharName . '","value":"' . $escapedCharValue . '"';
                                            $orConditions[] = $this->dl_filter_type . ':' . $this->filter_tv_names[$tvid] . ':regexp:' . $searchPattern;
                                        }

                                        // Объединяем значения одной характеристики через OR
                                        if (count($orConditions) > 1) {
                                            $characteristicConditions[] = 'OR(' . implode(';', $orConditions) . ')';
                                        } else {
                                            $characteristicConditions[] = $orConditions[0];
                                        }
                                    }

                                    // Объединяем разные характеристики через AND
                                    if (!empty($characteristicConditions)) {
                                        if (count($characteristicConditions) > 1) {
                                            $fltr .= 'AND(' . implode(';', $characteristicConditions) . ');';
                                        } else {
                                            $fltr .= $characteristicConditions[0] . ';';
                                        }
                                    }
                                } else {
                                    // Обычная обработка для не-MultiTV полей
                                ...
                                // Одиночное значение (не массив)
                                if (empty($v) || ($v == '0' && !isset($this->params['allowZero']))) {
                                    break;
                                }

                                if ($isMultiTVField) {

                                    // Даже для одного значения используем группировку
                                    $groupedValues = array();
        ...
        if (!empty($this->fp)) {
            ...
            if (is_array($f)) {
                foreach ($this->filter_tv_names as $fid => $name) {
                    ...
                    // проверяем, есть ли фильтр для текущего TV
                    if (isset($f[$fid])) {
                        // Строим фильтр БЕЗ текущего TV
                        foreach ($f as $tvid => $v) {
                            if ($tvid != $fid) {
                                $tvid = (int)$tvid;
                                $oper = 'eq';
                                ...
                                if ($isMultiTVField) {
                                    if (is_array($v)) {
                                        // Для множественного выбора используем regexp с OR
                                        $regexpParts = array();
                                        foreach ($v as $val) {
                                            $parts = explode('::', $val, 2);
                                            if (count($parts) == 2) {
                                                $charName = preg_quote($parts[0], '/');
                                                $charValue = preg_quote($parts[1], '/');
                                                $regexpParts[] = '"name":"' . $charName . '"[^}]*"value":"' . $charValue . '"';
                                            }
                                        }
                                        if (!empty($regexpParts)) {
                                            $val = implode('|', $regexpParts);
                                            $oper = 'regexp';
                                        }
                                    } else {
                                        $parts = explode('::', $v, 2);
                                        if (count($parts) == 2) {
                                            $charName = preg_quote($parts[0], '/');
                                            $charValue = preg_quote($parts[1], '/');
                                            $val = '"name":"' . $charName . '"[^}]*"value":"' . $charValue . '"';
                                            $oper = 'regexp';
                                        }
                                    }
                    ...
                    if ($fltr != '') {
                        // Есть другие фильтры - применяем их
                        $fltr = 'AND(' . $fltr . ')';
                        $DLparams['filters'] = $fltr;
                        $_ = $this->modx->runSnippet("DocLister", $DLparams);
                        ...
                        if (isset($f[$fid])) {
                            // Фильтр выбран, но других фильтров нет - получаем все товары
                            if (!$content_ids_list) {
                            ...
                            // Фильтр не выбран - используем all
